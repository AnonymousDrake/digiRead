...
const SearchResultScreen = ({
  ...
  const onSearchPress = () =>
    navigation.navigate('Search_Focus', {
      previousScreen: 'Search_Result',
    });

  const onViewBookPress = index =>
    navigation.navigate('Book_Info', {
      index,
      category: 'search',
      prevScreen: 'Search_Result',
    });
    ...
    <View style={styles.container}>
      ...
      {route.params.loading ? (
        ...
        <ResultCard
          data={data}
          onThresholdReached={() =>
            fetchMoreResults(searchString, filters, startIndex + maxResults)
          }
...
const mapDispatchToProps = dispatch => ({
  fetchMoreResults: (str, filter, index) =>
    dispatch(getSearchResults(str, filter, index)),
