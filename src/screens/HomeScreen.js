...
const HomeScreen = ({
  ...
  const onBookPress = (index, category) => {
    navigation.navigate("Book_Info", { index, category, prevScreen: "Home_Main" });
  }
  ...
  return (
    <View style={styles.container}>
      <HomeScreenHeader
        ...
        userEmail={userEmail}
        onFocus={() => navigation.navigate("Search_Focus", { previousScreen: "Home_Main" })}
        startLogout={startLogout}
      />
      {loading ? <LoadingBarFull /> : <Section data={data} fetchData={fetchBookshelf} onBookPress={onBookPress} />}
    </View>
  )
};
