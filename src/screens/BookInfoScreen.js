...
const BookInfoScreen = ({ route, navigation, searchResult, recommendedResult, continueResult, favoritesResult, myBooksResult, purchasedResult }) => {
  const bookIndex = route.params.index;
  const category = route.params.category;
  let data = {};

  if (category === "search")
    data = searchResult(bookIndex);
  else if (category === "recommended")
    data = recommendedResult(bookIndex);
  else if (category === "continueReading")
    data = continueResult(bookIndex);
  else if (category === "myBooks")
    data = myBooksResult(bookIndex);
  else if (category === "favorites")
    data = favoritesResult(bookIndex);
  else if (category === "purchased")
    data = purchasedResult(bookIndex);
  ...
  return <View style={styles.container}>
    <BookInfoHeader
      ...
      onPressBack={() => navigation.navigate(route.params.prevScreen, { loading: false })}
    />
    <BookInfoBody
      bookThumbnail={data.volumeInfo.imageLinks.thumbnail}
      bookTitle={data.volumeInfo.title}
      bookAuthor={data.volumeInfo.authors ? data.volumeInfo.authors[0] : undefined}
